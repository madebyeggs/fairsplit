class ApplicationController < ActionController::Base
  ...
  def bring_in_models
    ...
    @combined_sorted = (Announcement.all + Artist.where(:created_at => time_range) + Sound.where(:created_at => time_range) + Work.where(:created_at => time_range)).sort_by(&:updated_at)
